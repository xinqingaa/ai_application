...
TextChunk - 最基础的文本切片
SourceChunk - 后续系统真正消费的标准切片
DocumentCandidate - 文件发现阶段的判断结果
LoadedDocument - loader读取成功后的文档对象
MarkdownSection - md按照标题切分后的中间对象
ChunkDraft - 临时对象，用于文档切分到SourceChunk
SplitterConfig - 配置
DocumentPipelineResult - 汇总结果
"""
...
def main() -> None:
    ...
    print(f"已生成 {len(chunks)} 个 chunk")
    print("按文档统计：")
    ...
    for chunk in chunks[:3]:
        ...
        print(
            f"  文件名={chunk.metadata['filename']} "
            f"后缀={chunk.metadata['suffix']} ;加载器={chunk.metadata['loader']} "
            f"分块序号={chunk.metadata['chunk_index']}"
        )
        if "page_count" in chunk.metadata:
            print(f"  页数={chunk.metadata['page_count']}")
        if chunk.metadata.get("header_path"):
            print(f"  标题路径={chunk.metadata['header_path']}")
        print(
            f"  范围=({chunk.metadata['char_start']}, {chunk.metadata['char_end']}) "
            f" 字符数={chunk.metadata['chunk_chars']}"
        )
