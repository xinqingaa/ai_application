...
这一章建议你**在看服务层代码之前**，先把缓存策略想清楚。

原因是：

- 第七章已经把 `cache` 接进主链路
- 但当前实现是保守设计
- 如果你没有先理解“它为什么保守”，后面看到连续提问不命中缓存时就很容易误判成 bug
...
到了综合项目，缓存第一次不再是“孤立技巧”，而会直接影响：

1. 成本
2. 响应速度
3. 多轮上下文一致性
4. 会话状态设计
5. 后续 RAG / Agent 的扩展方式

所以缓存不应该等你看到 `stable_cache_key()` 才临时理解，而应该提前作为项目设计约束来说明。

### 当前项目的缓存属于什么类型

当前第七章的缓存，属于：

> **教学阶段的保守实现，也就是严格上下文缓存（strict context cache）。**

它的核心特点是：

- 只在 **provider、model、messages、temperature、max_tokens、json_mode** 都一致时才命中
- 优先避免错误复用
- 优先保证“这段回答确实属于当前这段上下文”

这意味着：

- 同一句问题，在多轮会话里不一定命中
- 中间如果插入了别的话题，后面即使再问回原问题，也通常不会命中
- 它更像“请求去重缓存”，而不是“按主题缓存”或“重复问题缓存”

### 为什么当前项目故意采用保守缓存

这类设计在教学项目里是合理的，因为这一章目前只有：
...
所以当前项目故意采用更保守的缓存策略，目的是：

1. 先把“缓存是一条主链路能力”建立起来
2. 先避免缓存把上下文复用错
3. 先让你看懂命中条件和 miss 的原因
4. 等后面引入 RAG / Agent 后，再把缓存分层细化

也就是说：

> 当前设计不是为了把命中率堆高，而是为了先把边界、风险和可解释性立住。
...
1. 多轮聊天上下文变化很快
2. 很多问题带实时性
3. 很多问题带用户权限、租户隔离、个性化配置
4. RAG 和 Agent 会把请求拆成更多中间步骤

所以企业级设计里，缓存往往是**分层的**，而不是“一个 dict 缓所有”。

### 企业级缓存一般至少分成哪几层

#### 第 1 层：请求去重缓存（Request Dedup / Strict Context Cache）

这层最接近当前项目的实现。

它的目标不是“尽量多命中”，而是：

> 避免在完全相同的请求上下文下，重复支付同一轮模型调用成本。
...
#### 第 2 层：单轮问答缓存（Single-turn Exact Cache）

这层的 key 通常不再包含整段历史，而只包含：
...
#### 第 3 层：语义缓存（Semantic Cache）

这层不是要求问题逐字相同，而是要求：

> 问题在语义上足够接近，且允许返回近似等价答案。
...
所以语义缓存通常不会作为第一层默认缓存，而会放在更受控的位置。

#### 第 4 层：RAG 中间结果缓存

一旦系统进入 RAG，很多收益其实不来自“缓存最终回答”，而来自缓存中间过程，例如：
...
这一层在真实业务里往往比“缓存最终 LLM 回复”更稳定、更高收益。

原因是：

1. 检索过程重复率很高
2. 检索结果相对更稳定
3. 比起直接缓存最终回答，更不容易产生错误上下文复用

#### 第 5 层：Agent 工具结果缓存

到了 Agent，缓存对象又会进一步变化。
...
> Agent 场景里，很多时候最该缓存的是工具结果，而不是最终模型话术。

### 企业级缓存设计时最重要的几个原则

#### 1. 先分层，再讨论命中率

不要一上来就问“为什么这个回答没命中缓存”。

应该先问：

1. 这是哪一层缓存？
2. 这个层的目标是安全还是命中率？
3. 这一层应该缓存最终回答，还是缓存中间结果？

#### 2. 权限、租户、实时性必须进 key 或直接禁用缓存
...
如果这些维度会影响结果，要么进入 key，要么直接不缓存。

否则就会出现最危险的问题：

> 不是缓存没命中，而是缓存命中了不该命中的结果。

#### 3. 被截断的回答不应该轻易复用

如果一次回答的 `finish_reason == "length"`，说明它可能是不完整的。

企业里通常不会把这种结果当成高质量缓存直接长期复用。
...
#### 4. Prompt / Model / Schema 变了，缓存就应该失效
...
- FAQ / 单轮问答缓存：小时级或天级
- 检索结果缓存：分钟到小时级
- 工具/API 结果缓存：按上游数据更新频率决定

如果一律给一个 TTL，看起来简单，实际很容易失控。
...
- last-user-only
- semantic cache
- answer reuse by intent

虽然命中率看起来会提高，但会把很多真正的业务问题提前隐藏掉。
...
- 默认只做严格上下文缓存
- 明确把 cache key 组成暴露在调试信息里
- 先让你理解“为什么会 miss”
- 后续在 RAG / Agent 章节再引入更细的缓存层

### 如果后面扩展到 RAG 和 Agent，缓存会怎样继续演进

一个更接近企业实现的演进路径通常是：

1. 保留当前 `strict_context` 作为最安全兜底层
2. 为单轮独立问题增加 `single_turn_exact` 缓存
3. 为知识库问答增加 RAG 检索结果缓存
4. 为高重复 FAQ 增加语义缓存
5. 为 Agent 工具调用增加工具结果缓存
6. 最后再根据业务指标决定哪些层值得长期保留

这也是为什么当前文档要明确说：

> **第七章的缓存实现不是企业最终形态，而是企业级分层缓存体系的起点。**

它现在偏保守，不是因为设计落后，而是因为：

- 当前阶段只有 LLM 主链路
- 还没有引入 RAG 和 Agent 的中间层
- 教学目标优先是“先看懂边界和风险”，不是“先把命中率堆高”
...
### 8.9 关于缓存，为什么这里不再展开重复说明

缓存策略的完整说明已经前置到了前面的独立章节 [缓存策略提前说明 📌](#缓存策略提前说明-)。

这里不再重复展开长篇讨论，只保留三个你在服务层里最需要记住的结论：

1. 当前项目默认采用的是**严格上下文缓存**，重点是安全和可解释，不是最大化命中率。
2. 当前项目之所以偏保守，是因为这一章还只有 LLM 主链路，尚未引入 RAG 检索层和 Agent 工具层。
3. 后续当项目扩展到 RAG / Agent 时，缓存会自然从“单层保守缓存”演进成“多层企业级缓存”。
...
1. 在 [chat_cli.py](../../source/02_llm/07_chat_cli/chat_cli.py) 里新增一个 `/history` 命令，直接打印当前会话消息历史。
2. 在 [llm_service.py](../../source/02_llm/07_chat_cli/llm_service.py) 里把第四章的严格 JSON 校验接进 `json_mode` 路径。
3. 在 [chat_api.py](../../source/02_llm/07_chat_cli/chat_api.py) 里增加一个清空会话的接口。
4. 在 [prompts/meeting_summary.txt](../../source/02_llm/07_chat_cli/prompts/meeting_summary.txt) 基础上，再写一个分类或提取模板，验证 `/prompt` 命令的价值。
5. 在 [llm_service.py](../../source/02_llm/07_chat_cli/llm_service.py) 里把 cache key 设计改粗或改细，观察命中率和风险。
