...
`03_rag` 是需求评审助手的知识系统与单 Agent RAG 主线。
...
- **本课**：`rag_core` / `rag_eval` / `rag_memory` 以**最小实现**跑通主链路（入库、检索、引用、拒答、单 Agent 等）；infra 可简化（如同步入库、轻量存储），能调试、能 eval 即可。
- **`06_ai_native`**：在**同一批 package** 上加深工程整合（FastAPI 分层、PostgreSQL / pgvector、Redis 任务、Docker Compose、Workbench 协议等），不另起平行实现。
- **依赖**：`import` 已有 `llm_core`（来自 `02_llm`），禁止 copy。

### 两层学习结构

**知识生产层**（专题 03–06）：对齐 RAGFlow 式 Context Layer 前半段——复杂资料如何经任务化入库、解析、表格处理、chunk、metadata、权限治理，被生产为可靠检索上下文。

**检索与助手层**（专题 07–17）：检索策略、上下文构造、生成与引用、评估、单 Agent 知识助手和课程收口（专题 17）。
...
├── 04_complex_document_and_table_processing.md
├── 05_chunking_and_metadata.md
├── 06_knowledge_governance.md
├── 07_embedding_and_vector_store.md
├── 08_retrieval_strategies.md
├── 09_query_rewrite_and_source_routing.md
├── 10_context_construction_and_compression.md
├── 11_generation_sources_refusal.md
├── 12_rag_evaluation.md
├── 13_rag_memory.md
├── 14_single_agent_rag_assistant.md
├── 15_rag_failure_analysis.md
├── 16_advanced_knowledge_production_cognition.md
├── 17_review_assistant_project.md
└── outline.md
```
...
- 建立 `source/packages/rag_core`（及本课相关 `*_core`）package 壳 + 首个 demo 占位（如 `03_minimal_rag`，00 以能 import、目录就绪为准）。
- 在文档中准备 3 段需求文档片段（作为后续入库/检索素材，00 不要求完整 RAG 流水线）。

### 主流框架实现

- LangChain Document / Retriever / Runnable（01 起在 demo 中落地）。
- Retriever as Tool、单 Agent 知识助手（本课后半）。
...
- 完整 golden set 建立与批量 eval（**03 评估专题 / 05_eval**）。
- 可运行的检索、引用、拒答链路（**01+** `03_minimal_rag` 等）。

### 完成标准（运行与观察）

- **代码**：`rag_core` 等 package 壳存在；知晓本课首个 demo 路径（见上方代码组织）。
- **文档**：能解释 RAG 与「只靠 Prompt」的区别；列出需求评审助手的知识源范围。
- **设计**：在笔记中草拟 3–5 条未来 golden set 问题类型（正常问答 / 无答案 / 需追问），不要求落库。

### 本节实战

定义需求评审助手的知识范围和单 Agent 能力范围（文档）：
...
## 04. Complex Document 与 Table Processing

### 真实问题

需求评审资料不只有 Markdown。PRD PDF、接口表格、会议纪要扫描件和原型截图的结构差异很大；解析质量直接决定 chunk、引用和检索是否可用。这是 RAGFlow 知识生产层最值得学习的部分之一。

### 基础原理

- 不同格式需要不同解析策略：Markdown、PDF 版面、表格字段、图片 OCR。
- 结构保留（标题层级、页码、section、表格列）是引用溯源的前提。
- 表格 chunk 与文本 chunk 的检索行为不同。
- 解析失败应进入文档状态和 bad case，而不是静默入库。

### 最小实现

- 解析一份含表格的接口文档和一份 PRD PDF（或降级样例）。
- 对比纯文本切分与保留表格结构的差异。
- 记录解析状态、chunk 预览和失败原因。

### 主流框架实现

- LangChain loaders、Unstructured 认知。
- RAGFlow DeepDoc / table path 设计思路（认知，不复制引擎）。
- 文档入库任务与状态更新。

### 失败分析与能力边界

- 扫描件、复杂版式 PDF、合并单元格表格解析不稳定。
- 图片中的需求说明当前阶段可认知，不强制实现多模态解析。
- 解析错误无法靠 rerank 或 Prompt 补救。
...
为需求评审助手准备复杂格式样例：含表格的接口文档、带章节的 PRD PDF、纯 Markdown 规则文档，并定义各类型解析边界。

### 本节沉淀

沉淀 `rag_core.parsing` 策略接口与解析失败归因规则。

## 05. Chunking 与 Metadata
...
- 知识库级、文档级、角色级权限模型。
- 检索前 ACL 过滤，避免越权召回与引用暴露。
- metadata 中的 owner、department、visibility 与前端 source panel 一致。
- 需求评审场景：产品库、接口库、规则库、历史评审库可按角色可见性划分。

### 任务化入库（Ingestion Job）

对齐 RAGFlow / MaxKB 的核心经验：文档先落库，解析与向量化异步执行；上传成功 ≠ 知识可检索。

- **Job 模型**：`IngestionJob` 绑定 Document，记录 `pending → parsing → chunking → embedding → completed / failed / cancelled`。
- **解耦原则**：API 只创建 Document + Job 并返回；worker 消费任务，更新 Document 与 Job 状态。
- **状态可见**：前端 knowledge workbench 必须能查看进度、失败原因、chunk 预览（见 `06_ai_native/07`）。
- **取消与重试**：支持 cancel in-flight job；失败 job 可 retry（从失败阶段或整链重跑，需在项目中二选一并写清）。
- **content_hash / digest**（认知）：同文件未变更时可跳过重复解析；当前阶段可选实现，不要求完整 digest 复用。
- **删除一致性**：删 Document 时同步清理 Chunk、Embedding、Job 记录，避免幽灵检索。

参考映射：RAGFlow TaskService + task_executor；MaxKB `embedding_by_document.delay()` + Document 多状态字段。

### 最小实现

- 设计 Knowledge、Document、Chunk、Embedding、IngestionJob、Citation、Feedback 的最小表结构。
- 跑通异步入库链路：

```text
upload → create Document + IngestionJob(pending)
      → worker: parse → chunk → embed → update status
      → completed（或 failed + error_msg，或 cancelled）
```

- 支持文档状态查询、失败重试、取消进行中的 job。
- 支持重新向量化（re-index）并产生新 job。

### 主流框架实现

- PostgreSQL 保存元数据。
- pgvector 保存向量。
- Redis + worker 处理解析和向量化任务。
- 知识库统计和状态面板。
...
- 同步阻塞式入库无法 cancel / retry，生产场景不可运维。
...
- 重建耗时和失败原因。
- job 成功率、cancel 成功率、retry 后成功率、各阶段平均耗时。
...
输出知识库领域模型、IngestionJob 状态机与最小管理能力；该链路在 `07_projects/04` 与 `06_ai_native/11` 继续落地。

## 07. Embedding 与 Vector Store
...
- 多知识库检索与合并。
- 权限过滤。
- **本项目 hybrid 方案**（对齐 MaxKB + 当前栈）：
  - 语义：`pgvector` similarity search。
  - 关键词：PostgreSQL 全文检索（`tsvector` / `to_tsquery`，或段落级 keyword 字段）；不引入 Elasticsearch 作为主线依赖。
  - 融合：向量分 + 全文分加权合并（权重可配置，写入 Knowledgebase 检索参数）。
  - rerank：可选 cross-encoder / LLM rerank；不能替代解析与 chunk 错误。
...
- LangChain Retriever / EnsembleRetriever 思路。
- PostgreSQL + pgvector + 全文检索（本项目默认路径）。
- BM25 独立引擎（如 rank_bm25）仅作对比实验，非生产默认。
- Reranker 认知（bge-reranker 等）。
- Hybrid Retriever 抽象。
...
- 为追 BM25 / ES 偏离 pgvector 主线，增加部署与数据一致复杂度。
- 全文与向量索引未同步更新会导致 hybrid 结果不可信（依赖 `03_rag/06` 任务化入库与删除一致性）。
...
## 09. Query Rewrite 与 Source Routing
...
## 10. Context Construction 与 Compression
...
## 11. Generation、Sources 与 Refusal
...
### 与 `05_eval_observability` 的分工

| 本专题（`03_rag/12`） | `05_eval_observability` |
| --- | --- |
| RAG **最小 eval**，服务 V2 质量基线 | **系统化**质量工程，服务 V4–V6 |
| 30 条 golden set、source hit、citation/refusal 核心指标 | Agent / Workflow eval、trace、版本回归、契约测试、质量面板 |
| 产出 `rag_eval` 包与 bad case 初版 | 接管并扩展 datasets、metrics、dashboard、feedback loop |

学完本专题应能跑通一次 RAG eval；**不必**在 `03_rag` 重复实现 `05` 的全套 trace / workflow eval。
...
## 16. Advanced Knowledge Production 认知

### 真实问题

当固定 RAG + hybrid retrieval 仍不够时，需要知道还有哪些知识生产与检索增强方案存在、解决什么问题、代价是什么。本专题只做边界认知，不要求实现。

### 基础原理

- 文档任务化入库：task、queue、cancel、retry、进度状态。
- digest 复用、重建索引、删除清理一致性。
- auto questions、auto keywords、auto metadata 等语义增强。
- parent-child chunk、TOC、长文档导航。
- RAPTOR、GraphRAG 适用边界。
- Connector / 外部数据源同步认知。

### 最小实现

- 为 5 个需求评审问题判断：固定 RAG、hybrid、parent-child、TOC、GraphRAG 或暂不升级。
- 画出任务化入库的状态流转图。

### 主流框架实现

- RAGFlow 知识生产层设计思路。
- MaxKB 文档多任务状态（EMBEDDING / SYNC）认知。

### 失败分析与能力边界

- 高级方案不能替代基础解析、chunk 和 eval。
- GraphRAG / RAPTOR 工程成本高，需求评审助手当前阶段不实现。
- 知识库文档多不等于需要 GraphRAG。

### 完成标准（运行与观察）

- 方案选型是否有 eval 依据。
- 升级前后 source hit 与成本对比。

### 本节实战

为需求评审助手写「高级方案不适用清单」，说明 V0–V3 为什么先不引入 GraphRAG / RAPTOR。

### 本节沉淀

输出高级知识生产认知备忘，供后续项目平台化时回看。
...
- **知识生产层**：任务化入库、复杂文档解析、chunk/metadata、删除清理、digest 复用、引用处理。
- **检索评测层**：hybrid retrieval、rerank、检索测试、评测与 bad case 回流。

### MaxKB（应用产品层，认知）

- Document / Paragraph / Embedding 多任务状态、PostgreSQL + pgvector、ChatRecord、反馈与 pipeline 递进思路；应用对象建模在 `07_projects` 落地。
