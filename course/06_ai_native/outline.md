...
- 不新建平行于 `02`–`05` 的 `*_core`；在**同一批 package** 上加深 FastAPI、数据模型、Redis 任务、Docker、SSE 协议与 UI 组件（`ai_ui_protocol` 等可为本课新增）。
- `03_rag` 阶段的最小 ingest / 检索实现，在本课升级为可运维的工程形态（与 `03_rag/06`、`11` 等呼应）。
- 全部通过 **import** 衔接；根 `review_assistant/` 作品化时复用同一批 `source/packages/`。
...
├── demos/                          # 默认 Vue 3 实现
│   ├── 06_streaming_state_sync/
│   ├── 06_schema_driven_ui/
│   ├── 06_structured_review_report_ui/
│   ├── 06_multi_agent_timeline_ui/
│   ├── 06_workflow_runtime_ui/
│   ├── 06_knowledge_workbench_ui/
│   └── 06_eval_feedback_ui/
├── apps/
│   └── review_assistant/
└── python_base/
...
`ai_ui_protocol` 等为本课新增 package，位于 `source/packages/`；业务 `*_core` 一律 **import** 加厚。根 `review_assistant/` 侧 Flutter 工作台复用 `ai_ui_protocol` 事件格式，不重复发明协议。
