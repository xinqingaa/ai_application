...
- 全部通过 **import** 衔接；`07_projects` 再将物理路径归拢到 `shared/`。
...
`ai_ui_protocol` 等为本课新增 package；业务 `*_core` 一律 **import** 加厚。`07_projects` 侧 Flutter 工作台复用 `ai_ui_protocol` 事件格式，不重复发明协议。
