...
第一阶段要交付需求评审助手的第一个可运行产品。固定“售后入口与订单状态”只作为稳定教学和评估 fixture；产品形态必须能从空 Project 冷启动，也能在已有基线项目上继续迭代。
...
真实产品的最小冷启动链路是：系统 member 创建 Project 并成为 owner，系统建立初始空 Brief，owner 或 editor 以标题和一句自然语言种子创建第一个 Requirement `draft`，再选择固定表单或导入 PRD。此时全局知识库或项目检索池为空是合法状态；ReviewRun 必须显示证据不足、Refusal 或补充问题，而不是把零候选当成系统失败。第一个 RequirementVersion 经人工批准并完成事务后索引，才开始沉淀为该项目后续运行可见的 `approved_requirement` 证据。
...
- 接受从空 Project、空 Brief、轻量 Requirement draft 到首个批准基线的冷启动契约；固定电商数据仅用于可重复验收，不是产品必须预置的资料。
...
第一阶段的核心不是追求高级检索，而是让每一层都可以观察和替换，并让评审结果落进一条有身份、有状态、能批准和交付的对象链。完整需求的产品判定已在本阶段锁定：除可选 `other` 外，每个固定分区必须有已确认内容、由人说明理由的不适用标记，或明确的待补充状态；待补充分区阻断批准，不能因为模型没有报告遗漏而放行。
