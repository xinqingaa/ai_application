...
- 明确用户身份认证的最小生命周期，以及普通评审 / 管理员两个角色各自能看到的路由和能执行的动作；这条边界不与第二阶段 Tool 权限混用。
- 明确知识资料上传后的暂存与发布流程：谁能触发入库、诊断信息如何展示、哪一个动作才会产生新的 `dataset_version`。
- 明确生成阶段结构化增量流式的事件契约：模型流式调用怎样被增量解析为局部字段、"生成中未校验"与"已校验完成"两种状态怎样在 SSE 事件和界面上区分、校验失败时增量内容如何被撤回。
...
`request_id` 标识一次运行，`requirement_id` / `requirement_version` 标识当前被评审对象，`requirement_text` 保存该版本的直接输入。`requirement_text` 可以来自用户粘贴的文本，也可以来自上传文件经文档加载器解析后的结果；两种输入方式共享同一个 `requirement_text` 契约，上传本身不代表这份内容进入知识检索候选池。`knowledge_scope` 只约束 Reference Knowledge 和 Historical Material 的可检索范围，不用于把当前 PRD 伪装成外部知识。
...
- 模型、Prompt、Retriever、Token、延迟和错误等诊断，错误至少区分模型鉴权失败、限流、超时、能力不支持、结构化校验失败与用户取消。

[可信生成](../../mechanisms/trusted-generation.md)先展示来源候选，并校验模型声明的来源是否属于本轮候选；后续 Citation 支持性机制继续判断内容能否支持声明，证据充分性机制再决定 Refusal 和补充问题。项目验收不能把“模型写出了来源编号”当成证据已经支持结论；同样不能把“生成过程中出现的增量内容”当成已经校验的结论——校验失败时，增量内容必须从界面上被显式撤回，不能只是停止更新。

第一阶段的产品交互至少区分 `idle`、`submitting`、`success` 和 `error`。生成阶段使用真实模型流式接口，后端按 `ReviewReport` Schema 增量解析已完整字段并通过 SSE 推送为“生成中、未校验”状态，生成结束后再执行一次完整 Schema 与 Citation 校验；这只是一次生成调用内部的增量渲染与最终校验边界，不是完整的 Agent Event 协议——没有多步骤循环、没有 Tool 调用事件、没有断线重连游标，SSE 连接断开时前端改用普通请求重新获取当前持久化状态即可，不需要恢复传输游标。完整的 Agent Event 协议、断线重连和运行轨迹留给第二阶段。
...
第一阶段只建设一个 Web 工作台，不建设 Flutter App，也不在 `source/apps/` 维护第二份产品页面。工作台按角色区分两块视图：评审视图（普通评审与管理员都可见）支持提交待评审需求（粘贴文本或上传文件）、按结构化增量流式查看生成过程、查看已校验的结构化风险、Citation、Refusal、补充问题、最终上下文摘要和真实错误；知识管理视图（仅管理员可见）支持上传知识资料、查看解析诊断、暂存与发布。诊断信息可以使用独立调试区域。这是一个真实但克制的最小管理后台——不做审批工作流、复杂权限矩阵或跨版本知识治理平台，但也不能只是能跑的脚本外壳。

### 7. 身份认证与角色边界

产品需要区分"谁在提交评审"和"谁在维护知识库"。第一阶段建立最小认证生命周期（登录、会话或令牌、失效与登出）和两个角色：普通评审只能提交与查看评审；管理员额外能上传知识资料、触发入库诊断和发布。这里的角色只回答产品界面上谁能点什么，是产品 RBAC，不是第二阶段 Tool 权限（模型能执行什么）；两者概念独立，不能相互替代或合并实现。

### 8. 知识资料的暂存与发布

管理员上传的知识资料先进入暂存状态，只有管理员执行明确的发布动作才会产生新的 `dataset_version` 并改变检索候选池；上传和暂存本身不生效。这条规则是为了保护 Golden Set 的冻结纪律——不能因为一次误传就悄悄改变已经冻结的验收基线。发布记录必须保留谁在什么时候发布了哪些文档，形成可追溯的版本历史。

### 9. 生成阶段的结构化增量流式

评审生成调用真实模型的流式接口，后端边接收 token 边做增量 JSON 解析，按 `ReviewReport` Schema 提取已经完整的字段并通过 SSE 推送给前端，标记为"生成中、未校验"；生成结束后再执行一次完整的 Schema 与 Citation 校验，前端用"已校验"的最终结果替换之前的增量展示。若最终校验失败，前端必须显式撤回增量内容，不能让未经校验的内容停留在界面上误导用户。这套流式契约只服务于一次生成调用，不引入 Tool 事件、多步骤循环或断线重连游标，与第二阶段完整的 Agent Event 协议保持边界。
...
- 身份认证、会话或令牌生命周期，以及普通评审 / 管理员两个角色的路由与动作边界。
- 知识管理后台：知识资料上传、解析诊断展示、入库暂存、管理员发布与 `dataset_version` 记录。
- FastAPI Review API 与结构化错误契约，包含结构化增量流式与最终校验的 SSE 事件契约。
- 最小 AI Native Web 工作台和请求状态，按角色区分评审视图与知识管理视图。
- 产品测试与最小评估样例。
...
- 认证所需的会话密钥或令牌签名密钥等环境变量，以及初始管理员账号的建立方式。
...
6. **身份认证与角色**：建立登录、会话或令牌生命周期，以及普通评审 / 管理员两个角色的路由与动作边界。
7. **Review API 与 Web**：组合唯一产品入口，完成请求状态、结构化增量流式与最终校验的 SSE 事件契约、风险、来源候选、上下文摘要和真实错误展示；按角色接入评审视图与知识管理视图（上传、暂存、发布）。
8. **评估与验收**：运行四路对照、真实 bad case / 策略边界、需求变更和实验前登记的质量门槛，形成第一阶段运行记录。
9. **环境打包**：用 Docker Compose 固定应用与 Postgres/pgvector 的本地运行环境，作为阶段验收的可复现环境收尾。
...
- 生成中（增量结果未校验）。
- 模型生成完成并已校验。
- 模型调用失败（细分鉴权失败、限流、超时、能力不支持）。
- 用户取消。
- 运行失败。

知识资料侧还要能区分：上传成功、解析诊断可用、暂存中、已发布（产生新 `dataset_version`）。SSE 连接断开是传输层事件，不改变上述任何一个业务运行状态。
...
- Reranker 进入产品默认链路。
- 审批工作流、超过两个角色的复杂权限矩阵、多租户和通用连接器。
- 完整的 Agent Event 协议：心跳、断线重连游标、Tool 事件和多步骤运行轨迹留给第二阶段；第一阶段的 SSE 只服务于一次生成调用的阶段状态和结构化增量。
- Redis 或后台任务队列：知识资料的暂存/发布先用数据库状态字段和 FastAPI 自带的后台任务承担，只有真实异步阻塞场景出现才引入。
...
- [ ] 最小工作台能提交需求（粘贴文本或上传文件）、展示结构化风险、Citation、Refusal、补充问题和来源候选，并区分运行、成功与真实失败。
- [ ] 用户能够登录，普通评审与管理员两个角色的可见路由和可执行动作有明确边界，且这条边界没有与第二阶段 Tool 权限混用。
- [ ] 管理员能够上传知识资料、查看解析诊断，并通过独立的发布动作产生新的 `dataset_version`；上传和暂存本身不改变检索候选池。
- [ ] 评审生成阶段有结构化增量流式展示，生成完成后的最终校验结果能够清晰替换未校验的增量内容；校验失败时增量内容被显式撤回，不会停留在界面上。
