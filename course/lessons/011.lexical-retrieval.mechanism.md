...
>
> 本文先建立“按词找”的直觉，再逐步认识 PostgreSQL 用来实现它的名称。数据库安装和实验命令见[Lexical Retrieval 实验篇](011.lexical-retrieval.lab.md)；若尚未通过学习路径中的 PostgreSQL 必备基础检查，先阅读[PostgreSQL 零基础](prerequisite.postgresql-for-ai-applications.concept.md)。
...
![Lexical Retrieval 从共享词法处理到候选排序](../assets/011.lexical-retrieval.overview.svg)
...
操作顺序、安装、启动、Role、Database、`.env`、migration 和固定资料入库见[Lexical Retrieval 实验篇](011.lexical-retrieval.lab.md)。正文只保留观察问题：
