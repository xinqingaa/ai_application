...
>
> 课程位置：[标准学习路径](../learning-path.md)。Embedding 阶段提供 Embedding、向量和相似度的前置直觉；Lexical Retrieval提供同一批资料上的词面检索对照，但不是本节的知识前置。本文解释机制和边界，不承担实验准备、完整命令或运行排障；这些内容见[Dense Retrieval 实验篇](012.vector-store-and-pgvector.lab.md)。

![Chunk 向量写入 pgvector 并参与 Dense Retrieval](../assets/012.vector-store-and-pgvector.overview.svg)
...
生产环境会有大量文档；那不改变这四个责任，只改变 Store 和 Index 的规模。问题和 Chunk 从哪里读、日志字段怎样对上，由[配套实验](012.vector-store-and-pgvector.lab.md)维护。
...
真实实验的准备、命令和依赖错误由[Dense Retrieval 实验篇](012.vector-store-and-pgvector.lab.md)维护。正文只要求你确认主路径使用真实 Embedding 和真实 PostgreSQL；缺少 key、migration 或 extension 时，应让错误暴露，而不是用本地假结果替代。
