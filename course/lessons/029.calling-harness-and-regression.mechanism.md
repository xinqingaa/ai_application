...
>
> 阅读顺序见[标准学习路径](../learning-path.md)。必要前提是 [Prompt Engineering](004.prompt-engineering.mechanism.md)、[Structured Output](005.structured-output.mechanism.md)、[可靠调用](006.reliability-and-errors.mechanism.md)，以及已经可以运行的固定 RAG 生成链；本文交付直接 LLM 与检索 RAG 可复用的 Case、Record 和 Summary。
...
完成实验后回到 [标准学习路径](../learning-path.md)。成本与缓存是 Harness 记录稳定后的 [按需支撑](030.cost-latency-and-caching.mechanism.md)。
