...
[可信生成](016.trusted-generation.mechanism.md)先展示来源候选，并校验模型声明的来源是否属于本轮候选；Citation 支持性机制继续判断内容能否支持声明，证据充分性机制再决定 Refusal 和补充问题；产品层再按 `finding_kind` 校验依据资格与目标成员资格。项目验收不能把“模型写出了来源编号”当成证据已经支持结论；同样不能把“生成过程中出现的增量内容”当成已经校验的结论——校验失败时，增量内容必须从界面上被显式撤回，不能只是停止更新。
...
- 解释 [LLM 在 AI 应用中的位置与边界](001.llm-in-ai-applications.concept.md)，知道固定 RAG 为什么仍然需要真实模型调用。
- 使用 [Prompt、Context 与 Schema 的模型契约](002.model-input-output-contracts.concept.md)描述一次评审调用的任务、证据和结果边界。
- 通过 [模型 API、Provider 与统一调用入口](003.model-api-and-provider.mechanism.md)运行真实模型。
- 使用 [面向应用的 Prompt Engineering](004.prompt-engineering.mechanism.md)和 [Structured Output 与本地校验](005.structured-output.mechanism.md)生成可被程序消费的风险结果。
