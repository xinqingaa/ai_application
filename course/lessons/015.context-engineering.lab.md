...
> 这是 [检索名单怎样变成模型本轮 Context](015.context-engineering.mechanism.md)的实验篇。它直接接住[固定 Retriever 控制与诊断实验](014.retriever-contract.lab.md)的真实 `RetrievalResult`，继续使用 `order_rules.md` 和 `surface_match=申请售后`，不更换材料制造更理想的 Context。
...
本实验承接[固定 Retriever 控制与诊断实验](014.retriever-contract.lab.md)。开始前确认：
...
本实验不提供这些上游参数作为主变量。要研究候选池和最终截断，应回到[固定 Retriever 控制与诊断实验](014.retriever-contract.lab.md)；这里从已经固定的 `RetrievalResult` 开始。
