...
> 这是 [Top-k、阈值、Metadata Filter 与 Retrieval 诊断](014.retriever-contract.mechanism.md)的实验篇。它继续使用第 11–13 节的同一份售后资料和同一个主题问题，观察两个真实 Chunk 怎样经过 `pre-filter → candidate_k → route threshold → RRF → final_top_k`。
...
本实验承接 [pgvector Dense Retrieval 实验](012.vector-store-and-pgvector.lab.md)和 [RRF 对照实验](013.multi-retrieval-and-rrf.lab.md)。开始前确认：
...
完成后回到[机制正文](014.retriever-contract.mechanism.md)，用真实输出核对两个 Chunk 在五层控制中的实际去向，再由[标准学习路径](../learning-path.md)进入[检索名单怎样变成模型本轮 Context](015.context-engineering.mechanism.md)。
