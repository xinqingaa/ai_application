...
第三章不是单纯把文本“变成一串数字”，而是要把第二章已经整理好的稳定文档块，变成一批可以和用户问题做语义比较的向量块。

最后你会得到的不是一堆孤立向量，而是一个可以继续往下走的结果：系统先把知识片段排好，再把用户问题排好，最后返回按相关程度排序的候选文档块。
...
第三章最终希望你看懂的是：系统怎样把这个问题变成一组数字，再拿它和每个文档块的数字表示逐一比较，最后把最相关的 `refund:0` 排在前面。

#### 先用一句自然语言记住主线

你可以先把第三章记成下面这句话：

> 先把文档块交给向量提供者变成文档向量(`document vector`)，再把用户问题交给同一个向量提供者(`vector provider`)变成问题向量(`query vector`)，最后用相似度把问题和文档块排出一个顺序。

#### 五个脚本分别代表什么阶段

| 阶段 | 脚本 | 这一阶段要看懂什么 |
|------|------|--------------------|
| 第 1 步 | `01_embed_chunks.py` | 第二章的 `SourceChunk` 怎么变成第三章的 `EmbeddedChunk` |
| 第 2 步 | `02_compare_similarity.py` | 用户问题怎么变成 query vector，再和文档向量比较 |
| 第 3 步 | `03_query_vs_document.py` | 为什么 query 和 document 需要两个入口，但仍然要落在同一个向量空间 |
| 第 4 步 | `04_real_embeddings.py` | toy provider 换成真实或 mock embeddings endpoint 之后，契约怎么保持不变 |
| 第 5 步 | `05_semantic_search.py` | toy provider 的局限在哪里，真实语义向量为什么更自然 |
| 复盘 | `tests/test_embeddings.py` | 哪些行为是第三章必须守住的不变量 |

#### 这些英文名字先怎么读

- `SourceChunk`：第二章留下来的稳定文档块
- `EmbeddedChunk`：带向量的文档块
- `embed_documents()`：把文档块正文变成文档向量
- `embed_query()`：把用户问题变成问题向量
- `cosine_similarity()`：计算两个向量像不像
- `ranked results`：按相似度排好的结果

#### 先按这个顺序读

如果你是第一次读第三章，建议先按下面的顺序建立骨架：

1. 先看这一节的总览，知道本章最终要产出什么
2. 再看 `01_embed_chunks.py`，理解文档块如何变成向量块
3. 再看 `02_compare_similarity.py`，理解问题如何参与排序
4. 再看 `03_query_vs_document.py`，理解两个入口为什么要保留
5. 再看 `04_real_embeddings.py`，理解真实 embeddings 接口补了什么
6. 最后再看 `05_semantic_search.py` 和测试，理解 toy provider 的边界和回归约束
...
第三章的五个脚本不是五个孤立 demo，而是沿着同一条主线逐层叠加。
...
01 只看文档块怎样带上向量
02 加入用户问题，开始做相似度排序
03 单独拆开 query/document 两个入口
04 把 toy provider 换成真实或 mock embeddings endpoint
05 对比 toy provider 和语义 provider 的排序差异
```

| 阶段 | 脚本 | 在主线中的位置 |
|------|------|----------------|
| 第 1 步 | `01_embed_chunks.py` | `SourceChunk[] -> EmbeddedChunk[]` |
| 第 2 步 | `02_compare_similarity.py` | `question -> query vector -> ranked results` |
| 第 3 步 | `03_query_vs_document.py` | 单独观察 query path 和 document path 的差异 |
| 第 4 步 | `04_real_embeddings.py` | 用真实或 mock provider 走同一套契约 |
| 第 5 步 | `05_semantic_search.py` | 看 toy provider 的已知缺口和真实语义向量的增量 |

先知道每个脚本站在哪一段，再去看代码里的函数名，阅读压力会小很多。

---

## 4. 第一层：`SourceChunk -> EmbeddedChunk` 📌

### 4.1 向量化的目标：把文本变成可比较表示
...
### 4.3 `EmbeddedChunk` 不替代 `SourceChunk`
...
## 5. 第二层：query vector 和 document vector 📌

现在先不要急着看真实 provider。第三章更重要的是先建立 query/document 分流进入向量层的判断标准。

### 5.1 query 和 document 在系统中的角色分工
...
## 6. 第三层：`embed_query / embed_documents` 📌

上一层先分清了 query vector 和 document vector 的角色。这一层继续看代码里为什么要保留两个入口。
...
### 6.3 `LocalKeywordEmbeddingProvider` 的教学定位
...
### 7.3 `embed_chunks()` 和 `score_query_against_chunks()` 的职责
...
### 7.5 `ensure_vector_dimensions()` 保护的维度契约
...
### 7.6 `ensure_same_embedding_space()` 保护的空间契约
...
### 8.2 与 `02_llm/02_multi_provider` 的边界
...
### 8.3 Embeddings 和 Chat Completions 的关注点差异
...
### 8.4 真实 provider 下的 `embed_query / embed_documents` 映射
...
### 8.5 `OpenAICompatibleEmbeddingProvider` 的能力范围
...
### 9.6 known gap 误排到 `refund:0` 的流程
...
回到开头，这一章你最终要能讲清楚的就是一条链路：

```text
第二章留下稳定文档块
-> 第三章把文档块变成 document vectors
-> 用户问题变成 query vector
-> query vector 和 document vectors 做相似度比较
-> 返回按相关程度排序的 chunk
```

如果你能解释下面几件事，就说明第三章的主线已经抓住了：

- `EmbeddedChunk` 是“原始 chunk 身份 + 向量”，不是只剩一串数字
- query vector 和 document vector 角色不同，但必须在同一个 embedding space
- `embed_query / embed_documents` 是接口语义分工，不一定对应两个真实 API
- `provider_name / model_name / dimensions` 是防止混用向量空间的契约
- toy provider 用来把机制讲清楚，真实 provider 用来接近生产语义效果
- known gap 和失败案例是复盘材料，不应该抢占第一次阅读主线
...
### A.7 第二步：`02_compare_similarity.py`
...
### A.8 第三步：`03_query_vs_document.py`
...
### A.10 第五步：`05_semantic_search.py`
...
### A.11 测试：`tests/test_embeddings.py`
...
### B.1 向量化是不是只是调用一下 embeddings API
...
### B.4 分开入口后，query/document 为什么还能比较
...
### B.6 toy provider 和真实 provider 到底是什么关系
...
### B.11 hash bucket 和角色区是不是也是向量设计方案
