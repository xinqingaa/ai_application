...
28. **固定程序、Workflow、Agent、Multi-Agent 与框架分层** · 待编写
    面对“步骤不能完全预先固定”时，先判断普通程序是否已经足够，并区分 Provider API、Agent 框架、状态运行时、本地 package 与产品领域组装。本节交付结构选择和责任分层，不把一次 SDK 调用误认为完整产品。
29. **LangChain 第一个真实 Agent 与最小 Loop 对照** · 待编写
    使用真实模型和一个确定性只读 Tool 直接运行 LangChain Agent，观察“模型决定—应用执行—结果回填—停止”的完整闭环；配套实验再用最小原生 Loop 对照框架承担的工作。本节结束时已经有可运行 Agent，而不是只积累底层零件。
30. **Agent Harness：框架运行时与应用控制面** · 待编写
    框架可以提供循环和调度，但上下文、权限、预算、停止、事件和产品责任仍需应用定义。本节建立 Harness 的责任图，明确哪些交给 LangChain，哪些留给本地治理层。
31. **Function Calling 与 Tool Schema** · 待编写
    模型生成的工具名和参数只是候选调用。本节解释 Tool Schema 怎样约束名称、参数和结果形状，以及它为什么仍不能代替权限和执行。
32. **Tool Runtime、执行生命周期与结构化错误** · 待编写
    本节跟踪一次 Tool Call 从参数校验、执行、结果转换到错误回填的完整生命周期，并说明框架 Tool 接口怎样映射到应用的稳定契约。
33. **Tool 权限、超时、取消、审计与副作用** · 待编写
...
35. **Retriever as Tool** · 待编写
    固定 RAG 的 Retriever 已有稳定输入、输出和诊断。本节把它接入框架 Tool Runtime，保留来源身份、空结果和路线失败，不让 Agent 绕过检索契约。
36. **最小 Run State、Event、预算与停止原因** · 待编写
    可运行 Agent 还需要留下可解释事实。本节只引入首个闭环所需的运行身份、步骤、Tool 调用、预算、基础事件与停止原因，为恢复和界面提供共同事实。
37. **LangSmith Trace 与本地 RunRecord** · 待编写
    本节使用 LangSmith 观察模型、Tool 与 Loop，并将托管 Trace 和本地运行记录通过稳定身份关联。Trace 用于定位运行过程，不替代产品日志、数据治理或质量验收。
38. **框架驱动的 `agent_core` 契约与提取边界** · 待编写
    在直接使用框架暴露真实复用后，本节才提取请求结果、Tool 治理、通用状态、停止原因、事件、Trace 和框架适配。`agent_core` 不重写 LangChain Loop，也不包含评审 Prompt、领域 Schema、允许路径或角色设计。
...
40. **Query Rewrite** · 待编写
    用户问题不一定适合直接检索。本节解释改写目标、原问题保留、技术标识保护和无新增信息边界，并要求每次改写可回查。
41. **Source Routing、补检索与追问** · 待编写
    Agent 要在内部 RAG、外部来源、工作区文件和用户补充之间选择信息入口。本节先建立路由、补检索和追问契约，具体外部 Tool 在后续接入。
...
46. **重试、副作用、补偿与幂等** · 待编写
    图恢复或重放不能重复外部行动。本节区分纯计算、可重试读取和有副作用操作，建立幂等键、执行记录和必要补偿。
...
62. **Token Stream 与结构化 Event Stream** · 待编写
...
66. **Agent Response State Machine 与运行界面** · 待编写
...
69. **Skill 格式、元数据、说明与资源组织** · 待编写
    Skill 用最小元数据声明用途和适用任务，再用说明、参考资料、脚本和资产组织能力。本节区分稳定约定、可选资源和客户端特有扩展。
70. **Skill 发现、激活、渐进加载与受治理执行** · 待编写
    本节跟踪发现、匹配、激活、说明加载、资源读取和脚本执行，处理 Context Budget、版本、兼容性和权限；脚本仍经 Tool Runtime、确认、沙箱和审计。
...
73. **Research Planning 与任务拆解** · 待编写
...
75. **Evidence Ledger、来源身份与证据积累** · 待编写
...
83. **框架协作模式：Subagent、Router、Supervisor 与 Handoff** · 待编写
...
87. **Multi-Agent 与单 Agent 基线检查点** · 待编写
    使用同一需求、工具、数据和预算比较质量、成本、延迟、证据一致性和失败定位；收益不足时保留单 Agent，不以角色数量证明能力。

## A2A 互操作

A2A 是独立协议与 SDK 生态，不是 LangChain 或 LangGraph 自带能力的别名。只有本地 Agent 契约稳定后，才把同一责任扩展到远程 Agent。

88. **A2A 解决的问题、角色、Agent Card 与能力发现** · 待编写
    区分 A2A Client、Remote Agent、本地 Delegation 和 MCP，解释 Agent Card 怎样声明身份与能力，并明确其中的 Skill 描述不等于 Agent Skills 文件格式。
89. **A2A Message、Task、Part 与 Artifact** · 待编写
    建立交互消息、可跟踪任务、内容片段和任务产物的关系，说明对话内容、执行状态和可交付结果分别由谁承载。
90. **A2A Task 生命周期、进度、输入、取消与错误** · 待编写
    跟踪任务提交、执行、等待输入或鉴权、完成、失败、拒绝和取消，并区分任务状态、流式更新和传输连接状态。
91. **A2A 协议绑定、安全、版本与真实 SDK 接入** · 待编写
    区分抽象数据模型与具体传输、鉴权和版本协商。配套实验固定官方规范与 SDK 版本，验证两个实现能否一致解释任务、产物、状态和错误。
92. **Multi-Agent 与 A2A 运行观测界面** · 待编写
    用户需要看到角色、远程任务、进度、局部结果、失败和冲突，而不是内部消息。本节把本地协作和远程任务映射为统一但不混淆的界面状态。
...
97. **Trace、Span、Run 与跨系统关联** · 待编写
    深化 LangSmith Trace、本地 RunRecord、LangGraph Checkpoint 和 A2A Task 的关联，说明 Trace 能帮助定位什么、不能替代什么质量判断；Langfuse 只在自托管或跨框架需求成立时做对照。
98. **Versioning、Experiment 与 Regression** · 待编写
    固定模型、Prompt、Schema、Retriever、Tool、Skill、框架、Workflow 和数据版本，建立运行身份、实验对照和回归规则，防止新结果覆盖旧证据。
99. **LLM-as-Judge 与 Human Eval** · 待编写
    自动 Judge 可以扩大评估规模，但会继承模型偏差。本节定义适用任务、评分契约、校准样例和人工复核，不让 Judge 替代产品验收责任。
100. **Bad Case 与 Feedback Loop** · 待编写
