...
框架不是附加在机制课程之外的第二条路线。每个框架单元都以一个产品问题为入口，同时解释公开运行模型、应用责任和失败边界；具体 SDK、版本与完整调用只进入配套实验。
...
28. **从固定 RAG 到 Agent：选择结构与实现层次** · 待编写
    当固定程序无法预先决定查询、工具或停止路径时，本节先判断哪里需要模型动态决策，再把 Provider API、LangChain、LangGraph、本地通用层和产品领域组装放入同一责任图。学完应能为当前任务选择最小可行结构，并说明每一层解决什么问题。
...
38. **用 `agent_core` 建立稳定契约与框架适配边界** · 待编写
    在 LangChain Agent、Tool Runtime 和最小运行事实已经跑通后，本节建立稳定的请求结果、Tool、Run State、停止原因、事件与 Trace 契约：当前 LangChain 通过下层适配器接入，评审领域组装位于产品上方；后续 LangGraph 也必须进入同一边界。只有出现跨入口复用、统一治理或隔离框架变化的真实需要时才提取这一层；否则产品继续直接使用框架。
...
90. **A2A Message、Task、Part 与 Artifact** · 待编写
