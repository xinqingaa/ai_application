...
| 项目掌握 | 能把机制组合进产品，处理真实失败和需求变化，并用测试或评估完成版本验收 |

标准学习路径负责安排概念、机制和实验的进入顺序；对应项目篇负责综合任务和版本验收。学习者不需要用文档数量计算进度，而应判断自己能否解释、验证、修改、调试和取舍当前版本的系统。

六个能力维度用于横向检查阶段一的 RAG 产品闭环是否完整；V3 的七层学习路径用于纵向建立单 Agent 从边界判断、状态与工具控制到质量评估和产品交付的能力。两者观察的是同一产品的不同侧面，不形成两套阅读顺序。

V0–V3 按同一产品逐步累积能力：

| 版本 | 累计建立的核心能力 | 完成后的能力定位 | 下一阶段继续解决的问题 |
| --- | --- | --- | --- |
| V0 固定 RAG 基线 | 知识生产、lexical / dense / RRF 检索、Context、结构化生成、API、最小工作台和最小质量证据 | 能构建可运行、可诊断、可比较的固定 RAG 应用 | 引用真实性、证据充分性和拒答 |
| V1 可信结构化评审 | Citation 校验、证据定位、Refusal、补充问题、知识版本和可信证据界面 | 能构建可引用、可拒答的可信 RAG 应用 | 系统化评估、回归和反馈治理 |
| V2 质量闭环 | Retrieval / Generation / Citation / Refusal Eval、Trace、Regression、Bad Case 和 Feedback | 能用质量证据持续改进 RAG 产品 | 查询、知识源和补检索步骤的动态决策 |
| V3 单 Agent RAG | Memory、Tool Runtime、Query Rewrite、Source Routing、Agent Loop、Guardrails、轨迹评估和运行界面 | 能构建受控、可观察、可评估的单 Agent RAG 应用 | 显式 Workflow 恢复与多 Agent 协作 |

这张表用于理解累计能力，不替代下面的标准阅读顺序，也不替代项目篇中的业务验收。
...
11. **[Lexical Retrieval、BM25 边界与 PostgreSQL 全文检索](mechanisms/lexical-retrieval.md)** · 可学习
    先从“有很多 Chunk，怎样按词找候选”建立直觉；遇到 Server、Database、migration 时按需补 [PostgreSQL 零基础](concepts/postgresql-for-ai-applications.md)，再按 [第 11 步实验准备](../source/demos/rag_retrieval_lab/docs/11-lexical-retrieval.md) 完成环境和第一次查询，最后回到正文把观察结果映射到 PostgreSQL FTS、BM25 和 `ts_rank`。产品使用 PostgreSQL FTS，不把 `ts_rank` 称为 BM25。
...
完成第 7–16 步后，已经建立 RAG 的核心机制链：资料从文件进入可追踪知识，lexical 与 dense 形成候选并由 RRF 融合，Retriever 与 Context Builder 各自承担选择责任，模型依据候选证据生成结构化结果。接下来把这条链交付为产品入口，并用固定样例验证质量、成本和失败边界。
...
完成 V0 后，应能够独立解释并修改固定 RAG 的知识生产、检索、Context 和生成链，比较直接 LLM、lexical、dense 与 RRF 路径，定位检索、上下文、生成和工程依赖故障，并通过 API 与最小工作台交付结果。V1 将在这条基线上把来源候选推进为经过应用校验的可信引用，并建立证据不足时的拒答与补充问题。
...
完成 V1 后，应能够区分检索候选、模型声明的引用和应用校验后的 Citation，让每项风险回到有效来源，并把证据充分、证据不足、拒答和需要补充转成稳定业务契约。V2 将继续建立可版本化的数据集、评估、回归和反馈，使质量改进能够被重复验证。
...
完成 V2 后，应能够分开评估检索、生成、Citation 和 Refusal，把模型、Prompt、Schema、Retriever 和数据版本关联到 Run 与 Trace，并将 bad case 和人工反馈转为可回归任务。此时固定 RAG 已具备持续迭代基础；V3 将把查询改写、知识源选择、补检索、追问和停止引入受控的运行时决策。
...
阶段二相关概念在 V3 用于选型和边界判断，真实实现按版本进入：

```text
V3：Single Agent + Memory + Tool Runtime + Agent Loop
V4：Workflow State + Checkpoint + Human-in-the-loop
V5：Multi-Agent 责任分工、协作与冲突处理
```

### 第一层：边界判断

1. **Chain、固定 RAG、Workflow、Agent 与 Multi-Agent 边界** · 待编写
   用固定 RAG 的真实失败证明哪些步骤需要模型动态决策，并判断何时保持 Chain、何时使用单 Agent、何时应进入后续 Workflow 或 Multi-Agent；V3 产品只实现单 Agent。
...
5. **Function Calling 与 Tool Schema** · 待编写
   将模型提出的行动约束为可校验的工具调用草案，明确工具参数和返回结果的 Schema、必填字段、类型、业务约束与校验失败行为。

### 第四层：工具运行时

6. **Tool Runtime 与结构化错误** · 待编写
   由应用执行工具、校验输入输出并暴露真实失败。
7. **工具权限、高风险确认、超时、幂等与审计** · 待编写
   明确模型提出行动与应用校验、确认、执行和记录之间的边界。

### 第五层：Agent Loop 与动态 RAG
...
完成 V3 后，应能够判断一个问题应保持 Chain 或固定 RAG，还是进入单 Agent、Workflow 或 Multi-Agent；区分 Run State、Conversation、短期记忆、长期偏好和业务知识；通过 Tool Schema 校验工具输入输出，并由 Tool Runtime 执行权限、确认、超时、幂等、审计和结构化错误处理；解释 Agent 每次继续、完成、追问、失败、达到上限或安全阻止的原因；评估工具选择、参数、轨迹、记忆污染、质量、成本和延迟。产品界面能够正确处理 SSE、取消、重连和响应状态，并展示 Tool Call、记忆使用、证据变化、停止原因和真实失败。

V4 在此基础上增加显式 Workflow State、Checkpoint、Interrupt、Resume 和 Human-in-the-loop；V5 再增加多个 Agent 的真实责任边界与协作。
...
| 版本 | 暂定结果 |
| --- | --- |
| V4 可控 Workflow | 将运行过程建模为显式 State、节点和分支，增加 Checkpoint、Interrupt、Resume、Human-in-the-loop，以及对应运行时界面 |
| V5 多 Agent 评审 | 在 Workflow 基础上增加真实角色契约、共享状态、并行执行、失败隔离、汇总、冲突裁决，以及多 Agent 协作界面 |
| V6 产品化 | 整合并完善已有工作台和质量面板，补齐本地或个人私有服务器的单用户部署、演示和作品化；核心能力随 V0–V5 已逐步建立，本版本不扩展多租户与企业合规平台 |
