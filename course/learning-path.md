...
本文是课程阅读顺序和课程序号的唯一真源。当前课程只有第一阶段、第二阶段和一套从 1 到 92 的连续编号。
...
34. **MCP 解决的问题、角色、协议版本与责任边界** · 待编写
    每个外部能力都写专用适配会重复协议接入和能力描述。本节解释 MCP 解决什么连接问题，区分 Host、Client、Server，并建立协议版本、外部能力与内部治理的边界。
35. **MCP 能力发现、能力模型与内部 Tool Runtime 映射** · 待编写
    外部 Tool、Resource、Prompt 等能力需要先被发现和描述，再映射成应用内部可治理对象。本节解释能力描述、Schema、信任和来源怎样进入 Runtime，同时说明能力存在不等于可信或已经获得执行权限。
36. **MCP 请求生命周期、传输、错误与真实接入** · 待编写
    本节以正文标明的当前规范修订跟踪发现、请求、结果、错误和取消，区分协议层无状态请求、业务显式状态和旧版会话机制。配套实验固定协议与 SDK 版本，接入真实只读 MCP 能力并观察版本或能力不兼容怎样显式失败。
...
49. **Skill 格式、元数据、说明与资源组织** · 待编写
    Skill 需要用最小元数据声明名称、用途和适用任务，再用说明、参考资料、脚本和资产组织可复用能力。本节区分开放格式的稳定约定、可选资源与客户端特有字段，不把目录存在当作能力已经安全可用。
50. **Skill 发现、激活、渐进加载与受治理执行** · 待编写
    本节跟踪 Skill 从发现、任务匹配、激活、说明加载到按需读取资源和执行脚本，处理 Context Budget、版本、兼容性和权限。脚本仍通过 Tool Runtime 或等价受治理边界执行，不能因来自 Skill 而绕过确认、沙箱和审计。
51. **最小 Agentic RAG 与 Skill 项目检查点** · 待编写
...
70. **Multi-Agent 拆分判断** · 待编写
    本节判断客户端影响、接口契约和证据审查是否具有独立责任与收益，区分并行子任务、普通函数和真正 Agent 角色。
71. **角色、上下文、工具与输出契约** · 待编写
    每个 Agent 必须说明负责什么、不负责什么、能看到什么、能调用什么以及交付什么，避免共享全部上下文后产生角色幻觉。
72. **Supervisor、Worker 与 Delegation** · 待编写
    本节解释任务怎样被委派、拒绝、重派和回收，Supervisor 为什么负责组合与停止而不自动拥有所有领域判断。
73. **共享状态、私有上下文与证据** · 待编写
    Agent 可以共享任务和已批准证据，但私有推理、局部上下文和工具权限不能无边界扩散。本节建立共享与隔离规则。
74. **并行、依赖、取消与失败隔离** · 待编写
    本节处理可并行任务、依赖任务、全局取消和局部失败，防止一个 Worker 失败后汇总器把缺失结果解释为无风险。
75. **结果合并、证据归属与冲突裁决** · 待编写
    不同 Agent 的风险项可能重复、冲突或引用不同版本。本节保留来源与责任人，区分可自动合并和必须暴露的分歧。
76. **A2A 解决的问题、角色、Agent Card 与能力发现** · 待编写
    本地 Agent 契约成立后，A2A 才用于跨边界交换任务和状态。本节区分 A2A Client、Remote Agent、本地 Delegation 和 MCP，解释 Agent Card 怎样声明身份与能力，并明确其中的 Skill 描述不等于 Agent Skills 文件格式。
77. **A2A Message、Task、Part 与 Artifact** · 待编写
    远程交互中的消息、可跟踪任务、内容片段和任务产物不能混用。本节建立四类核心对象的关系，说明对话内容、执行状态和可交付结果分别由谁承载。
78. **A2A Task 生命周期、进度、输入、取消与错误** · 待编写
    本节跟踪任务提交、执行、等待输入、等待鉴权、完成、失败、拒绝和取消，并区分任务状态、流式更新和传输连接状态。
79. **A2A 协议绑定、安全、版本与真实互操作** · 待编写
    相同任务语义可以映射到不同协议绑定。本节区分抽象数据模型与具体传输、鉴权和版本协商，配套实验固定规范与 SDK 版本，验证两个实现能否一致解释 Task、Message、Artifact、状态和错误。
80. **Multi-Agent 运行观测与协作界面** · 待编写
    用户需要看到角色、进度、局部结果、失败和冲突，而不是一串内部消息。本节把协作状态映射为可理解的运行界面。
81. **Multi-Agent、A2A 与基线检查点** · 待编写
    使用同一需求、工具、数据和预算比较单 Agent 与 Multi-Agent 的质量、成本、延迟、证据一致性和失败定位，收益不足时保留单 Agent。至少把一个已有责任在本地 Delegation 与远程 A2A 路径间做同契约对照，验证互操作没有改变证据归属和最终责任。

## 必要 Workflow

Workflow 不单独成为项目阶段，也不建设低代码画布。它只为需要显式状态、持久化、恢复、人工确认和副作用治理的路径提供确定性骨架。

82. **Workflow State、Node 与状态转换** · 待编写
    本节把必须稳定执行的步骤表达为显式状态和转换，区分模型决策与确定性节点，并定义节点输入输出和合法分支。
83. **Checkpoint、持久化与 Resume** · 待编写
    长任务或外部等待不能只存在内存中。本节解释哪些状态需要持久化、怎样从检查点恢复，以及代码或配置版本变化后的恢复边界。
84. **Interrupt 与 Human-in-the-loop** · 待编写
