...
3. **[Model API、调用生命周期与 Provider 抽象](mechanisms/llm/model-api-and-provider.md)** · 可学习
...
4. **[面向应用的 Prompt Engineering](mechanisms/llm/prompt-engineering.md)** · 可学习
   把临时提示词变成可命名、可版本化、可比较的任务协议。
5. **[Structured Output 与应用侧校验](mechanisms/llm/structured-output.md)** · 可学习
   用 Schema、解析和业务校验决定结果能否进入程序。
6. **[Reliability、错误分类与可见降级](mechanisms/llm/reliability-and-errors.md)** · 可学习
...
12. **[Context Engineering：输入装配、预算与证据边界](mechanisms/llm/context-engineering.md)** · 等待前置
    Retriever 先产生候选证据，Context Builder 再决定本轮模型真正看到什么。
13. **可信生成、Sources、Citation Candidate 与证据不足** · 待编写
    约束模型只依据候选证据生成，并区分来源候选、真实引用和拒答。

这里刻意把 LLM 与 RAG 交叉起来：Context Engineering 位于 Retriever 之后，而不是因为文件存放在 `mechanisms/llm/` 就提前学习。

### 第四段：建立可比较基线

14. **[LLM Calling Harness 与最小回归](mechanisms/llm/calling-harness-and-regression.md)** · 等待前置
...
- **[Streaming、事件协议与 Conversation](mechanisms/llm/streaming-and-conversation.md)** · 按需支撑
  当产品需要 SSE、增量渲染、取消或会话历史时进入。必要前置是 Provider。
- **[Token、成本、延迟与缓存边界](mechanisms/llm/cost-latency-and-caching.md)** · 按需支撑
