...
- 第 1–16 节是已经形成稳定数据流的冻结区，不在其中插课或改序。
...
第 1–16 节已经形成稳定顺序和真实实验。后续重构不改变这些编号，也不把新的基础课插入其中。
...
本节吸收可信 RAG 输出边界，不建立独立“可信评审版本”，也不扩展完整 Citation 运营平台。
...
第二阶段项目篇在真正进入实现前创建，不预建空文档。它将维护同一个阶段契约，并在第 42、57 和 65 节提供三次返回检查点；这些检查点不是新版本。
...
38. **[SSE 结构化事件协议](mechanisms/streaming-and-conversation.md)** · 待重切
    从现有 Streaming 正文中保留 Token Stream、Event Stream、SSE 和稳定事件字段，承载 Tool Call、检索、等待、停止与错误。
