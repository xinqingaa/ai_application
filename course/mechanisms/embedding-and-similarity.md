...
>
> 课程位置：[标准学习路径](../learning-path.md) V0 第十步。必要前置是 [RAG 与外部知识的边界](../concepts/rag-and-external-knowledge.md)；建议先完成 [文档加载](document-loading-and-cleaning.md) 与 [Chunking](chunking-and-metadata.md)。本文交付真实 Embedding 调用与成对相似度观察；分数只说明表示空间中的接近程度，不证明事实正确，也不代替后续的匹配、排名与证据判断。
...
先把每个向量想成空间中的方向。语义接近的句子，方向更靠近；无关句子更分散。真实模型输出通常是成百上千维的稠密向量，但阅读时先抓住三件事：
