...
有了“文本先进入同一向量空间、分数还要带方向”的直觉，用 RAG 总图标一下本篇停在哪一段。下面只定位，不开始讲怎样从资料里找出候选：
...
本篇停在 `SimilarityObservation`：先学会读一对文本的分数。向量怎样留下来、下次怎样再拿出来和许多资料比，现在不必展开。
...
真实链路中，Chunk 文本和用户说法会进入同一个表示空间。这里固定少量规则句、查询改写和噪声句，是为了隔离“表示是否合理”这一变量，而不是用手写字符串替代真实知识库。

本篇比较的是你这次送进模型的句子之间的距离。兼容 embeddings 端点是表示服务：送来几段文字，就返回几条向量。它不替你记住哪一段是哪个 Chunk，下次提问也不会自动去翻已有资料。把问题拿去和一批已经留下来的资料比远近，是后面的问题。

## 用真实相似度验证直觉

实验固定若干句对，先预测哪些更接近，再观察真实空间中的排序、分数方向和模型差异。单个句对相似不等于已经从许多资料里找出该看的几段，也不证明生成结论有依据。运行、输出和读码路径见[配套实验](../labs/embedding-and-similarity.md)。

## 将自然边界、输入错误和真实服务故障分开

主路径缺少密钥、鉴权失败、限流、超时、404 端点或供应商异常时，应看到清晰的 `LLMError`，而不是空向量或本地假分数。

建议按输入、配置、Provider 和空间身份逐层定位：
...
处理：承认表示边界；后续还要用词面信息、上下文选择和证据判断补齐，而不是要求 Embedding“理解法律逻辑”
```
