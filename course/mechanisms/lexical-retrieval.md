...
- 哪些行进入候选？
- 两行都匹配时谁排前面？
- 查询写“发起逆向服务”、资料写“申请售后”时，为什么可能返回 0 个候选？
- 怎样区分“查询成功但没有相同词”和“数据库连接或 SQL 执行失败”？
- 排名高的 Chunk 是否真的能支持最终风险结论？

先给出直观答案：lexical retrieval 更像“按词查找”。它能找到相同词语和精确标识，但不会自动知道“逆向服务”和“申请售后”意思相近。数据库成功执行查询后返回 0 条，是一种正常查询结果；数据库连接或 SQL 真正失败时，代码会返回明确错误，而不是 0 条候选。后文会用同一数据库里的精确词查询做对照。
