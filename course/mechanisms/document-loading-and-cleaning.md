...
>
> 课程位置：[标准学习路径](../learning-path.md)第一阶段第 8 节。必要前置是 [RAG 与外部知识的边界](../concepts/rag-and-external-knowledge.md)。本文用 PDF 建立复杂文档的完整心智模型，真实代码实现 TXT、Markdown、DOCX 和文本型 PDF；本文不产生 Chunk，不建立索引，也不实现 OCR/VLM。
...
只要得到文本，结果还会包含 `pdf_reading_order_not_guaranteed`。warning 表示内容可以继续处理但必须核对；error 表示当前第一阶段路线没有产生可继续处理的内容。
...
实验会先展示一个可重复的双栏 PDF 阅读顺序对照：视觉左栏应先读，但 fixture 的 PDF 内容流先记录右栏，抽取结果因此先出现右栏。这是用于隔离 Parser 行为的受控对照，只能证明内容流与视觉阅读顺序可能不一致，不能冒充真实业务文档分布或第一阶段产品质量证据。
...
无文本层 PDF 的失败不是“PDF 没有结果”，而是一个有行动含义的路由结果：当前文本抽取路线已经证明不适用。由于第一阶段没有配置真实 OCR/VLM 服务，系统在这里停止并暴露边界。
