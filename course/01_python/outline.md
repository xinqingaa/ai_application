...
> **定稿说明**：本课程为**已完成的 Python 基础**，不作为 `02_llm` 之后主线的扩展重点；学习组织方式与 [learning-guide.md](../../docs/learning-guide.md) 中的「一节一交付 / 单包 import」规范**不适用**于本目录（历史练习型结构保留）。主项目学习从 `02_llm` 起按 learning-guide 执行。
...
| 阶段 | 章节 | 内容 | 重要程度 |
|-----|------|------|---------|---------|
| **阶段一** | 1-4 | 基础语法 + 数据结构 | ⭐⭐⭐ 必须掌握 |
| **阶段二** | 5-8 | 函数 + 类 + 模块 | ⭐⭐⭐ 必须掌握 |
| **阶段三** | 9-12 | HTTP + 异步 + FastAPI | ⭐⭐ 重要 |

### 学习策略

**前期：认真打基础**

- 目标：熟悉 Python 语法规范，建立基本认知
- 方式：逐行理解代码，动手敲一遍
- 范围：第 1-2 节（环境配置、基础语法、数据结构）

**中期：理解 + 实践**

- 目标：掌握函数、类、模块等核心概念
- 方式：理解原理，复制代码运行，修改参数观察效果
- 范围：第 3-8 节

**后期：快速过 + 项目驱动**

- 目标：能调用 API、写 FastAPI 服务
- 方式：直接复制代码片段运行，快速掌握用法
- 范围：第 9-12 节
- 重点：开始接入 LLM API

### 前端开发者快速对照

你已经熟悉的 concept，只需要学习 Python 语法：

| 概念 | JavaScript | Python |
|-----|-----------|--------|
| 变量 | `let name = "张三"` | `name = "张三"` |
| 函数 | `function add(a, b) {}` | `def add(a, b):` |
| 异步 | `async/await` | `async/await`（语法相似） |
| 类 | `class Person {}` | `class Person:` |
| 模块 | `import` / `export` | `import` / `from ... import` |
...
- 遇到问题再查文档
- 边做边学
- 适用：第 9-12 节，开始接入 LLM API 时

**建议**：前期认真学习，建立 Python 基础认知后，可以逐渐切换到快速过或项目驱动模式。
