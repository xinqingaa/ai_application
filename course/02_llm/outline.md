...
- OpenAI SDK 风格调用（本课 00 demo 直接使用）。
- OpenAI 兼容平台：`base_url` + 同一 SDK（配置细节在 01 `models.yaml`）。
- 后续 LangChain / LangGraph 通过 `llm_core` 统一接口复用（01 起）。
...
- `LLMClient`、Provider 抽象、多平台分步配置教程（**01**）。
- 完整请求/响应控制台、统一结构化日志（**01** `LLMResponse`）。
- 样例批量回归、JSONL 落盘、harness 对比（**07**）。
...
- OpenAI SDK（封装在 `OpenAICompatProvider`）。
- OpenAI 兼容平台（`.env` 中 `OPENAI_BASE_URL`；README 含 DeepSeek 步骤）。
- LangChain 将来读取同一 YAML（认知即可）。

### 失败分析与能力边界

- OpenAI 兼容不代表 structured / tool / stream 全支持。
- `LLMErrorCode` 分类；完整重试在专题 06。

### 本节不做

- streaming（04）、Structured Outputs API（03）、embedding 调用、`schema` 成功率统计、harness 落盘（07）。
...
- `embedding.default_embed` — 预置，03_rag 使用。
