...
这些材料用于机制观察，不是冻结的第一阶段 acceptance 数据集。Lexical Retrieval 与 Dense Retrieval 使用的 Chunk 仍由 ingestion fixture 经过真实 Loader 与 Chunker 产生，不在这里复制一份平行索引数据；问题 JSON 不是知识库原文。
