...
这个脚本用于观察结构化输出的约束层级，不用于证明某个 mode 永远最好。

读代码时关注：

1. `PROMPT_VERSION = "4.0.0"`：三种 mode 共用同一 Prompt。
2. `MODES`：`prompt_only` / `json_mode` / `json_schema`。
3. `client.chat_structured(...)`：调用后立刻 parse。
4. `result.parse.ok` / `error_stage`：判断输出是否可被业务消费。
