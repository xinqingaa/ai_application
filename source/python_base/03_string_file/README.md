...
> 本文档说明如何跟着 [学习文档](../../../course/python_base/03_string_file.md) 一步步动手操作
...
```
source/python_base/03_string_file/
├── README.md            ← 你正在读的这个文件
...
```bash
python source/python_base/03_string_file/string_advanced.py
```
...
```bash
mkdir -p source/python_base/03_string_file/data
```

然后手动创建 `source/python_base/03_string_file/data/example.txt`，内容随意写几行，例如：
...
# 方式 1：读取全部内容
file = open("source/python_base/03_string_file/data/example.txt", "r", encoding="utf-8")
content = file.read()
print(content)
file.close()
...
# 覆盖写入
file = open("source/python_base/03_string_file/data/output.txt", "w", encoding="utf-8")
file.write("Hello World\n")
file.write("第二行")
file.close()

# 追加写入
file = open("source/python_base/03_string_file/data/output.txt", "a", encoding="utf-8")
file.write("\n这是追加的内容")
file.close()

# 验证：读取看看写入是否成功
file = open("source/python_base/03_string_file/data/output.txt", "r", encoding="utf-8")
print(file.read())
file.close()
```
...
base = "source/python_base/03_string_file/data"
...
先手动创建 `source/python_base/03_string_file/data/log.txt`，内容如下：
...
再手动创建 `source/python_base/03_string_file/data/config.txt`，内容如下：
...
```bash
python source/python_base/03_string_file/file_io.py
```
...
base = Path("source/python_base/03_string_file/data")
...
```bash
python source/python_base/03_string_file/pathlib_basics.py
```
...
```bash
python source/python_base/03_string_file/json_basics.py
```
...
# 创建 example.txt
cat > source/python_base/03_string_file/data/example.txt << 'EOF'
这是第一行
这是第二行
这是第三行
Hello World
...
# 创建 log.txt
cat > source/python_base/03_string_file/data/log.txt << 'EOF'
2024-01-15 10:30:15 INFO User logged in
2024-01-15 10:31:20 ERROR Database connection failed
2024-01-15 10:32:00 INFO Request completed
EOF
...
# 创建 config.txt
cat > source/python_base/03_string_file/data/config.txt << 'EOF'
# 应用配置
API_KEY=your_api_key
MODEL=gpt-4
MAX_TOKENS=1000
...
# 创建 4 个练习用的 .py 文件
touch source/python_base/03_string_file/string_advanced.py
touch source/python_base/03_string_file/file_io.py
touch source/python_base/03_string_file/pathlib_basics.py
touch source/python_base/03_string_file/json_basics.py
```
