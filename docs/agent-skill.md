...
- **能力目录化**：当 package 根目录开始堆积多个单文件能力时，优先按职责拆成子包；不要继续追加 `harness.py`、`cost.py` 这类会变胖的根级文件。
...
- 新 demo 目录不是默认选项；先复用或扩展已有 lab。只有已有 lab 无法表达新的观察维度时，才新增 demo lab。
...
学习型运行入口包括 demo、app、CLI 脚本、API 服务、workflow runner 和 eval runner。它们的目标是帮助用户理解真实机制，不只是验证脚本能跑。因此默认应遵守：

- 主路径使用一条短命令运行，实验变量优先放在文件顶部的中文配置开关中。
- 每个配置开关必须能从代码注释或 README 看懂：控制什么、默认为什么、什么时候改。
- 课程和项目运行入口主路径默认调用真实模型、真实外部服务或真实本地服务；fake / mock / simulation 只用于单元测试、离线排查、稳定复现失败路径或明确标注的对照实验。
- 如果入口提供 `USE_REAL_LLM`、`CALL_LLM`、`USE_REAL_SERVICE` 等开关，默认值应为真实调用；缺少 key、模型能力不支持或供应商异常应清晰暴露，不应静默退回模拟结果。
- 终端输出要区分输入、应用侧诊断、模型结果和最终结论，避免用户跑完后找不到「结果在哪里」。
- 学习入口默认只打印到终端或展示在本地 app，不把运行结果写入 `/tmp`、用户目录或项目外路径。
- 如果确需保存导出结果，必须提供显式开关，并写入被 `.gitignore` 忽略的本地输出目录。
- 不把核心算法写在运行入口里；入口只加载样例、选择配置、调用 `source/packages/` 的共享能力、打印或展示观察结果。
