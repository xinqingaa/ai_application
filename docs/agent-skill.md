...
学习型 demo 的目标是帮助用户理解机制，不只是验证脚本能跑。因此默认应遵守：

- 主路径使用一条短命令运行，实验变量优先放在文件顶部的中文配置开关中。
- 每个配置开关必须能从代码注释或 README 看懂：控制什么、默认为什么、什么时候改。
- 课程 demo 主路径默认调用真实模型或真实外部服务；fake / mock / simulation 只用于单元测试、离线排查、稳定复现失败路径或明确标注的对照实验。
- 如果 demo 提供 `USE_REAL_LLM`、`CALL_LLM` 等开关，默认值应为真实调用；缺少 key 或供应商异常应清晰暴露，不应静默退回模拟结果。
- 终端输出要区分输入、应用侧诊断、模型结果和最终结论，避免用户跑完后找不到「结果在哪里」。
- demo 默认只打印到终端，不把运行结果写入 `/tmp`、用户目录或项目外路径。
- 如果确需保存导出结果，必须提供显式开关，并写入被 `.gitignore` 忽略的本地输出目录。
- 不把核心算法写在 demo 脚本里；demo 只加载样例、选择配置、调用 `source/packages/` 的共享能力、打印观察结果。
