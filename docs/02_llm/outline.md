...
| Claude Sonnet 4 | 200K | 长文本优秀、代码能力强 |
| Claude Opus 4 | 200K | 最强推理 |
| DeepSeek-V3 / R1 | 128K | 开源、性价比极高 |
| Gemini 2.x Pro | 1M+ | 超长上下文 |
| 通义千问 | 128K | 国内可用、中文优秀 |
...
2. **API 调用方式**
   - 大多兼容 OpenAI 格式（DeepSeek、通义千问等可直接用 OpenAI SDK）
   - SDK 使用
...
3. **选择策略**
   ...
   - 中文能力
   - 价格对比
   - 开源 vs 闭源（DeepSeek 开源可本地部署）

#### 实战案例

```python
# 1. 使用 DeepSeek API（兼容 OpenAI 格式）
from openai import OpenAI
...
# 完成一次调用，体验与 OpenAI 的差异

# 2. 使用 OpenAI 兼容接口调用国内模型
# DeepSeek、通义千问等都支持 OpenAI SDK 格式
...
2. **实现方式**
   ...
   - **原生 Structured Outputs**（推荐，直接传 Pydantic Schema）
   - Pydantic 手动验证

3. **原生 Structured Outputs** 📌
   - OpenAI：`client.beta.chat.completions.parse(response_format=Model)`
   - Claude：Tool Use 模式实现结构化输出
   - 优势：模型保证输出符合 Schema，无需重试
   - 限制：并非所有模型都支持
...
person = response.choices[0].message.parsed  # 直接得到 Pydantic 对象

# 4. 对比三种方式
# Prompt 指定格式 vs JSON Mode vs Structured Outputs
# 在可靠性、易用性、模型兼容性方面的差异
```
...
1. **多模态输入**
   - 图片 + 文本混合输入
   - 支持的模型：GPT-4o、Claude Sonnet/Opus、Gemini
   - 图片格式：URL、Base64
...
# 3. Claude Vision 调用
# 对比 OpenAI 和 Claude 的多模态 API 差异
```
...
2. **成本优化策略**
   ...
   - 缓存常见请求
   - 压缩 Prompt
   - **Prompt Caching**（系统提示和 few-shot 缓存）

3. **Prompt Caching** 📌
   - Anthropic Prompt Caching：缓存系统提示，减少 90% 输入成本
   - OpenAI Prompt Caching：自动缓存重复前缀
   - 适用场景：固定 system prompt + few-shot 的高频调用
   - 实现方式：Anthropic 需显式标记 cache_control，OpenAI 自动处理
