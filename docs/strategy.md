...
第一阶段不建设 Agent、Multi-Agent、通用 Workflow 或完整质量平台；需求版本的持久状态迁移全部由人的显式动作触发，评审运行、知识解析与索引状态由固定程序推进且不自动改变需求版本状态。结束时 Retriever 必须形成稳定 Tool 契约，需求对象模型必须能被第二阶段 Agent 以同一写入路径复用。
