...
这一层显性对齐 RAGFlow 式**知识生产层**：复杂资料如何经任务化入库、解析、chunk、metadata、权限治理，被生产为可靠检索上下文；详见 `course/03_rag/outline.md`。
...
### 6. AI Native 体验与项目工程层

这一层解决 AI 应用如何承接模型流式输出、RAG 证据链、Agent 工具轨迹、Workflow 运行态和多模态任务，同时补齐当前项目所需的 FastAPI、PostgreSQL / pgvector、Redis、后台任务和本地部署等基础工程能力。

它不是重新学习传统前端，也不是转向纯后端工程，而是围绕 AI 应用特有的协议、状态、运行时、动态 UI、Agent UX、知识工作台、评测反馈界面和项目工程化能力沉淀 AI Native 应用能力。
...
当前主项目是需求评审助手（从 RAG 知识助手演进为多 Agent + Workflow 评审系统）。现阶段不并行维护第二项目，避免学习战线过长。
...
这是两条**独立但相关**的轴，文档与大纲中不混为一谈：

| 轴 | 含义 | 载体 |
| --- | --- | --- |
| **能力路线** | 按主题系统学习 AI 应用能力 | 上文七层能力 + `course/02_llm`–`07_projects` |
| **项目版本** | 需求评审助手产品逐步完善 | V0–V6，见下文「项目方向」 |

学某一门课或某一节，是在积累**能力**；VX 标记的是**产品**可演示、可验收的里程碑。二者交叉引用，不一一对应。
...
`python_base` 和 `02_llm` 代表已经完成或需要补强的基础层。`02_llm` 后续应覆盖模型交互与上下文工程，不只是 API 调用入门。

`03_rag` 是 RAG、LangChain 与单 Agent 知识助手主线。它不只学习固定 RAG，也要把需求评审助手做到可引用、可拒答、可评估，并具备查询改写、知识源选择、检索质量判断和追问补全等单 Agent 能力。前半侧重 RAGFlow 式知识生产，后半侧重检索上下文与知识助手。

`04_agent` 是 Agent 工作流与工具系统主线，系统学习 Function Calling 深入、Tool Runtime、Agent Loop、LangChain Agent、LangGraph Workflow、Human-in-the-loop、Agentic RAG 深化、Deep Research 认知、Multi-Agent、Agent Skills、MCP 和开放 Agent 协议。

`05_eval_observability` 是评估、可观测与质量工程主线。它既服务 RAG，也服务 Agent、Workflow 和项目，重点是 golden set、trace、bad case、回归测试、工程契约测试、成本延迟指标和线上反馈闭环。

`06_ai_native` 是 AI Native 体验与项目工程主线，承接前端 / Flutter / 跨端优势，也补齐 FastAPI、Redis、Docker、后台任务、本地部署等项目必需工程能力。重点沉淀 SSE 状态同步、Schema 驱动交互、Agent UX、Workflow Runtime UI、知识工作台、评测反馈界面和 AI 应用工程化能力。

`07_projects` 围绕需求评审助手收敛，不再无限扩展零散项目。根目录 `review_assistant/` 作为可部署产品真源，**import** `source/packages/` 中的共享包，形成可运行、可展示的作品。

历史非主线补充资料见 [archive/](../archive/)。

## 文档与代码关系

文档建设优先于代码堆叠，但代码仍然是学习闭环不可缺少的部分。

后续不再要求每个章节都对应多个脚本。**学习认知链路**与**单节交付**以 [learning-guide.md](learning-guide.md) 为准：

```text
A. 学习认知链路
真实问题 -> 基础原理 -> 最小实现 -> 主流框架实现 -> 失败分析与能力边界

B. 单节交付
本节实战 -> 完成标准（运行与观察）-> 本节沉淀
```

更合适的代码组织方式是：

- 专题文档负责讲清真实问题、基础原理、实现路径、失败边界和完成标准。
- 示例代码负责验证关键机制，不追求覆盖每一个知识点。
- **每个能力域一个 package 实例**（位于 `source/packages/`），全仓库 **import 复用**；`03_rag` 最小实现跑通，`06_ai_native` 工程加深，`07_projects` 在根 `review_assistant/` 作品化。
- Demo 负责验证单个能力组合。
- App 负责收敛为完整项目，当前主项目是需求评审助手。
...
这个项目服务于企业内部需求评审和知识辅助场景，围绕 PRD、业务规则、接口文档、会议纪要和历史评审记录，输出带依据的问答、结构化评审结果、缺失信息追问和风险提示。从 RAG 知识助手逐步演进为多 Agent + Workflow 评审系统。

它主要用于打牢 RAG、LangChain 与单 Agent 知识助手能力，并按版本递进演进：

- V0：固定 RAG 问答。
- V1：Sources / Refusal / 结构化评审输出。
- V2：检索评估、bad case 与 feedback 回流。
- V3：单 Agent 增强，包括查询改写、知识源选择、追问补全。
- V4：多 Agent 评审协作。
- V5：Workflow / Human-in-the-loop，包括风险审核、状态流转和人工介入。
- V6：AI Native 工作台、质量面板、本地部署与作品化演示。
