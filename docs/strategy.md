...
`07_projects` 围绕需求评审助手收敛，不再无限扩展零散项目。将各课散落的共享 package **物理归拢**到 `source/07_projects/shared/`（import 整合，非 copy 重写），形成可运行、可展示的作品。

`99_foundation` 是非主线知识补充区。LLM 原理、微调、私有化部署、LangChain 底层抽象等内容只在 RAG / Agent / 项目学习中遇到具体理解障碍时回看，不作为前置课程。

`00_archive/` 仅为过渡保留，与新课程无学习路径关系；新内容以 `docs/` 下当前规范为准。
...
- **每个能力域一个 package 实例**（如 `llm_client`、`rag_core`、`agent_core`），全仓库 **import 复用**；`03_rag` 最小实现跑通，`06_ai_native` 工程加深，`07_projects` 路径归拢。
- Demo 负责验证单个能力组合。
