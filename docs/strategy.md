...
### 6. AI Native 前端、Agent UI 与生成式交互层

这一层解决 AI 应用前端如何承接模型流式输出、RAG 证据链、Agent 工具轨迹、Workflow 运行态、多模态任务和企业平台配置。

它不是重新学习传统前端，也不是只做聊天框、卡片和后台页面，而是围绕 AI 应用特有的协议、状态、运行时、动态 UI、Agent UX、多模态交互和平台化治理沉淀前端能力。
...
这一层是当前职业定位的差异化优势：不是普通前端页面能力，而是把 LLM、RAG、Agent、Workflow、Eval 和多模态任务变成可配置、可运行、可干预、可追踪的 AI 产品体验。

### 7. 项目平台化与作品化层

这一层解决如何把前面能力收敛为两个递进项目，并逐步形成企业级 AI 应用平台雏形。

长期维持两个项目更适合学习推进和简历表达，但二者不是割裂关系。需求评审 RAG 助手 / 智能客服是可独立展示的 RAG + 单 Agent 项目，也可以作为金融 Copilot 中的知识型子 Agent / 子模块复用。
...
- 可视化工作流编排
- MCP / 工具生态平台
- Agent Runtime Observability Center
- 数据标注与模型评测平台
- 权限、审计、安全、成本治理
...
`06_ai_native_frontend` 是 AI Native 前端、Agent UI 与生成式交互主线，承接前端 / Flutter / 跨端优势，重点沉淀 SSE 状态同步、Schema 驱动交互、Generative UI、Agent UX、Workflow Runtime UI、多模态文档智能体和企业 AI 前端架构。

`07_projects` 围绕两个长期主项目收敛，不再无限扩展零散项目。项目一是可独立展示的 RAG + 单 Agent 项目；项目二复用项目一能力，扩展为金融场景的多 Agent Copilot 与企业级 AI 应用平台雏形。项目代码和文档可以逐步吸收 `03_rag`、`04_agent`、`05_eval_observability`、`06_ai_native_frontend` 的阶段成果。
...
- App 负责收敛为完整项目，例如需求评审 RAG 助手和金融 Copilot。
...
这个项目面向金融客服、运营、审核、合规和知识运营场景，提供一个能理解问题、引用依据、识别风险、辅助决策和支持人机协作的 AI 工作台。

它可以在项目一基础上扩展，逐步加入多 Agent 协同和完整产品化能力：

- V0：金融知识问答 Copilot。
- V1：合规审核 Copilot。
- V2：公告 / 财报 / 资讯解读。
- V3：多 Agent 协同，包括客服、检索、审核、总结、人工审批。
- V4：观测、评估、权限、审计、部署和作品化演示。
