...
本仓库面向前端、Flutter 和跨端开发者，通过唯一项目“需求评审助手”学习 AI 应用开发。第一阶段建立固定 RAG，第二阶段演进为 Agent、Tools 与 Multi-Agent 系统。
...
- 课程分为概念篇、机制篇、实验篇和项目篇。
- 机制篇讲原理、数据或状态变化和边界，不讲实现代码。
- 实验篇位于 `course/labs/`，负责初始化、运行、日志、调试、读码和验证。
- 项目篇引用 `SPEC.md`，负责综合实践和学习验收，不复制产品规格或运行手册。
